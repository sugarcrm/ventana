    ...
    function SugarApi(args) {
        ...
        return {
            ...
            exportRecords: function(params, $el, callbacks, options) {
                ...
                var self = this;
                if (params.sample) {
                    record_list_id = sample;
                } else {
                    var recordListUrl = this.buildURL(params.module,'record_list')
                }
                return this.call('create',recordListUrl,{'records':params.uid},
                          {complete: 
                           function(rawResponse) {
                               var response = JSON.parse(rawResponse.xhr.responseText);
                               self.fileDownload(
                                   self.buildURL(response.module,'export/'+response.id), 
                                   callbacks, 
                                   { iframe: $el});
                           }
                          }
