...
SUGAR.Api = (function() {
    ...
    function SugarApi(args) {
        ...
        var _handleErrorAndRefreshToken = function(self, request, callbacks) {
            return function(xhr, textStatus, errorThrown) {
                ...
                if (self.needRefreshAuthToken(request.params.url, error.code)) {
                    ...
                    crosstab.once('auth:refresh:complete', refreshCallbacks.complete);
                    crosstab.once('auth:refresh:success', refreshCallbacks.success);
                    crosstab.once('auth:refresh:error', refreshCallbacks.error);
        ...
        return {
            ...
            search: function(params, callbacks, options) {
                ...
                var method = 'read';
                if (options && options.fetchWithPost) {
                    method = 'create';
                }
                var url = this.buildURL(null, 'globalsearch', null, params);
                return this.call(method, url, data, callbacks, options);
            },
    ...
    return {
        ...
        createInstance: function(args) {
            ...
            'crosstab' in window && crosstab.on('auth:refresh', _.bind(function() {
                ...
                this.login(null, { refresh: true }, {
                    complete: function() {
                        crosstab.broadcast('auth:refresh:complete');
                    },
                    success: function() {
                        delete self._runningRefreshToken;
                        crosstab.broadcast('auth:refresh:success');
                    },
                    error: function() {
                        delete self._runningRefreshToken;
                        crosstab.broadcast('auth:refresh:error');
                    }
                });
            }, _instance));
