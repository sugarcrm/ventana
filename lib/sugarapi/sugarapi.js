...
SUGAR.Api = (function() {
    ...
    function SugarApi(args) {
        ...
        return {
             ...
             * @param {String} query
             * @param {String} fields (comma delimited). If null or empty, all fields will be returned from endpoint.
             * @param {String} modules (comma delimited)
             * @param {Integer} max_num Max search query result set size. Should be obtained from client application's config. (optional - defaults 10)
             * @param {Object} callbacks hash with with callbacks of the format {Success: function(data){}, error: function(data){}}
             */
            search: function(query, fields, moduleList, max_num, callbacks) {
                var params, method, payload, url;
                // Caller may omit max_num, in which case second to last input is callbacks + set max_num default
                if(!callbacks) {
                    callbacks = max_num;
                    max_num = 10;
                }
                ...
                method = 'search';
                payload = {};
                url = this.buildURL(null, method, null, params);
