    ...
    function SugarApi(args) {
        ...
        var _handleErrorAndRefreshToken = function(self, request, callbacks){
            return function(xhr, textStatus, errorThrown) {
                ...
                else if (_externalLogin && 
                         error.status == 401 && 
                         error.payload && 
                         error.payload.url && 
                         error.payload.platform == _platform)
                {
                    self.handleExternalLogin(error, onError);
                }
                else {
                    onError();
                }
        ...
        return {
             ...
             * - passDownloadToken: Boolean flag indicating if download token must be passed in the URL (`false` by default)
            ...
            ping: function(action, callbacks, options) {
                return this.call(
                    'read',
                    this.buildURL('ping', action, null, { platform: _platform }),
                    null,
                    callbacks,
                    _.extend({
            ...
            handleExternalLogin: function(error, onError) {
                var r, authData;
