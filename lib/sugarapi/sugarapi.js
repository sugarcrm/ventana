...
SUGAR.Api = (function() {
    ...
    var HttpError = function(request, textStatus, errorThrown) {
        ...
        if (this.responseText && ((this.status == "400") || (this.status == "401"))) {
            try {
                this.oAuthCode = JSON.parse(this.responseText).error;
            }
            catch(e) {
                console.log("Failed to extract OAuth2 error from response: " + this.responseText + "\n" + e);
            }
        }
    };
    ...
    _.extend(HttpError.prototype, {
        ...
        isAuthTokenExpired: function() {
            return this.oAuthCode == "invalid_grant";
        },
    ...
    _.extend(HttpRequest.prototype, {
        ...
        /**
         * Checks if a request is OAuth2 request.
         * @return {Boolean} Flag indicating if this request is executed against OAuth2 endpoint.
         */
        isOAuth: function() {
            // TODO: This is not 100% reliable way to detect if it's auth request
            return this.params.url.indexOf("/oauth2/") > 0;
        }
    });
        ...
        return {
            ...
            call: function(method, url, data, callbacks, options) {
                ...
                params.error = function(xhr, textStatus, errorThrown) {
                    ...
                    // Attempt to refresh token only if the request is not authentication request
                    if (error.isAuthTokenExpired() && self.canRefreshAuthToken() && !request.isOAuth()) {
             ...
             * @param {Object} params properties: q, moduleList, fields, max_num, offset. The query property is required.
             * { q: query, moduleList: commaDelitedModuleList, fields: commaDelimitedFields, max_num: 20 },
